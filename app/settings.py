...
class Settings(BaseSettings):
    ...
    TELEGRAM_CHAT_WHITELIST: str = Field(
        default="", description="Allowed chat IDs, can simultaneously constrain channel, group, private, supergroup."
    )
    ...
    DEV_MODE_MOCKED_TEMPLATE: str = Field(
        default="<b>in the dev mode!</b>", description="When development mode is enabled, this template will be returned as a reply."
    )

    # Telegraph configuration
    TELEGRAPH_SHORT_NAME: str | None = Field(
        default=None, description="Short name of the Telegraph account, displayed above the edit button"
    )
    ...
    BINDERS_YAML_PATH: Path = Field(
        default=PROJECT_DIR / "config" / "binders.yaml", description="Binders configuration file path"
    )

    SUPER_ADMIN_ID: int = Field(default=0, description="Super admin Telegram User ID, has all permissions")
    ...
    SHA256_SALT: str = Field(
        default="❤TelegramBot❤", description="SHA256 hash salt value, used for encrypted storage of user IDs"
    )
    ...
    def model_post_init(self, context: Any, /) -> None:
        ...
        if "linux" in sys.platform:
            if self.ENABLE_DEV_MODE:
                logger.warning("Development mode has been automatically turned off, please do not run development mode on Linux")
                self.ENABLE_DEV_MODE = False

            if self.ENABLE_TEST_MODE:
                logger.warning("Test mode has been automatically turned off, please do not run test mode on Linux")
                self.ENABLE_TEST_MODE = False

        # In test mode, automatically turn off development mode and force blocking mode
        if self.ENABLE_TEST_MODE:
            if self.ENABLE_DEV_MODE:
                logger.warning("Development mode has been automatically turned off, development mode and test mode cannot be enabled simultaneously")
            self.ENABLE_DEV_MODE = False
